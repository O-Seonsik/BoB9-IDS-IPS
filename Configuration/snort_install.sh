...
mkdir ~/snort_src
cd ~/snort_src
apt-get install -y build-essential autotools-dev libdumbnet-dev libluajit-5.1-dev libpcap-dev zlib1g-dev pkg-config libhwloc-dev make liblzma-dev openssl libssl-dev cpputest libsqlite3-dev uuid-dev
apt-get install -y libtool git autoconf bison flex libcmocka-dev libnetfilter-queue-dev libmnl-dev libnetfilter-queue-dev libmnl-dev
cd ~/snort_src
...
make
make install
apt-get install -y libnuwind-dev
cd ~/snort_src
